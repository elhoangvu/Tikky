...
NSString *const kGPUImageRainDropFragmentShaderString = SHADER_STRING
(
 ...
 void main() {
     ...
     float fade = S(0., 0., T);                            // fade in at the start
...
@interface GPUImageRainDropFilter () {
    GLuint _resolutionUniform;
    GLuint _timeUniform;
}

@property (nonatomic) CGFloat timeDelta;
@property (nonatomic) GPUVector3 resolution;
@property (nonatomic) CADisplayLink* displayLink;
@property (nonatomic) NSInteger lastTextureIndex;

@end

@implementation GPUImageRainDropFilter

- (instancetype)init {

    if (!(self = [super initWithFragmentShaderFromString:kGPUImageRainDropFragmentShaderString])) {
        return nil;
    }
    
    self.stillImage = YES;
    self.timeDelta = 100000;
    self.speed = 30.;
...
- (void)setResolution:(GPUVector3)resolution {
    _resolution = resolution;
    
    [self setVec3:_resolution forUniform:_resolutionUniform program:filterProgram];
}
...
- (void)setInputSize:(CGSize)newSize atIndex:(NSInteger)textureIndex {
    ...
    GPUVector3 vec3;
    vec3.one = newSize.width;
    vec3.two = newSize.height;
    
    self.resolution = vec3;
