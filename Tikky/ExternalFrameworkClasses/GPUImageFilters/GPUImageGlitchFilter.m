...
NSString *const kGPUImageGlitchFragmentShaderString = SHADER_STRING
(
 ...
 vec4 taylorInvSqrt(vec4 r)
{
 ...
 void main(void){
     ...
     float y = textureCoordinate.y * resolution.y;
     float rgbWave = (
     ...
     float rgbDiff = (6.0 + sin(time * 500.0 + textureCoordinate.y * 40.0) * (20.0 * strength + 1.0)) / resolution.x;
     ...
     float whiteNoise = (random(textureCoordinate + mod(time, 10.0)) * 2.0 - 1.0) * (0.15 + strength * 0.15);
...
@interface GPUImageGlitchFilter () {
    GLint _glitchUniform;
...
- (instancetype)init
{
    if (!(self = [super initWithFragmentShaderFromString:kGPUImageGlitchFragmentShaderString]))
    {
        return nil;
    }
    
    _glitchUniform = [filterProgram uniformIndex:@"time"];
    _resolutionUniform = [filterProgram uniformIndex:@"resolution"];
